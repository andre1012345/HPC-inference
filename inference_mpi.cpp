// =============================================================================
// Parallel Ensemble Inference Engine — MPI Data Parallelism
// HPC Project · Andrea · University of Messina
//
// Data parallelism at process level: N test samples distributed across p ranks.
// Each rank independently runs the full ensemble on its N/p local samples.
// No communication during inference — embarrassingly parallel workload.
//
// Pattern:
//   MPI_Scatter  → distribute X_test chunks from Rank 0 to all ranks
//   local inference → each rank processes N/p samples independently
//   MPI_Gather   → collect predictions at Rank 0
//
// Timing: starts at MPI_Scatter, ends after MPI_Gather.
// File I/O and model broadcast are excluded from timing.
//
// Compile: mpic++ -O2 -fopenmp -o inference_mpi inference_mpi.cpp
// Run:     mpirun -n 4 ./inference_mpi
// =============================================================================
...
// -----------------------------------------------------------------------------
// I/O helpers — only Rank 0 calls these
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// Model inference functions — thread-safe, called from parallel region
// -----------------------------------------------------------------------------
...
int main(int argc, char** argv) {
    ...
    // Alternative: Rank 0 loads and broadcasts — here we keep it simple.
    std::vector<double> X_train_v    = load_matrix(MODEL_DIR+"/X_train.txt", N_TRAIN, N_FEATURES);
    std::vector<double> y_train_d    = load_vector(MODEL_DIR+"/y_train.txt");
    std::vector<double> nb_means_v   = load_matrix(MODEL_DIR+"/nb_means.txt",  N_CLASSES, N_FEATURES);
    std::vector<double> nb_vars_v    = load_matrix(MODEL_DIR+"/nb_vars.txt",   N_CLASSES, N_FEATURES);
    std::vector<double> nb_priors_v  = load_vector(MODEL_DIR+"/nb_priors.txt");
    std::vector<double> lr_weights_v = load_vector(MODEL_DIR+"/lr_weights.txt");
    std::vector<double> lr_bias_v    = load_vector(MODEL_DIR+"/lr_bias.txt");
    ...
    // ── Load test set and ground truth on Rank 0 only ────────────────────────
    long   n_test = 0;
    ...
    // Broadcast n_test so all ranks know the partition size
    MPI_Bcast(&n_test, 1, MPI_LONG, 0, MPI_COMM_WORLD);

    long local_n = n_test / size;   // samples per rank (assumes N divisible by p)
    int  local_elems = local_n * N_FEATURES;  // doubles per rank
    ...
    // Pre-allocate KNN distance buffer (1 per thread, single-threaded MPI version)
    std::vector<std::pair<double,int>> dist_buf(N_TRAIN);
    ...
    for (int run = 0; run < N_RUNS; ++run) {
        ...
        // ── MPI_Scatter: distribute X_test from Rank 0 ───────────────────────
        // sendcount = local_elems = (N/p) * D doubles sent to each rank
        ...
        // ── Local inference — each rank processes its N/p samples ─────────────
        // No communication with other ranks during this phase.
        ...
        // ── MPI_Gather: collect predictions at Rank 0 ────────────────────────
        // Each rank sends local_n integers; Rank 0 assembles global array.
        MPI_Gather(
    ...
    if (rank == 0) {
        ...
        std::cout << "\n========================================\n";
        std::cout << "  MPI DATA PARALLELISM RESULTS\n";
        std::cout << "========================================\n";
        std::cout << "  Ranks       : " << size         << "\n";
        std::cout << "  N_test      : " << n_test        << "\n";
