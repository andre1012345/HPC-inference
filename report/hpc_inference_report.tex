...
Logistic Regression). The ensemble is trained offline on a fully synthetic
binary classification dataset (70{,}000 samples, 20 features) using a
Python training script (\texttt{train.py}) and achieves 88.44\% accuracy.
...
pipelines. When one of the models is a brute-force K-Nearest Neighbours
...
with 20 features, this amounts to roughly $2 \times 10^{10}$ floating-point
operations, making sequential execution too slow for any interactive use case
(baseline wall time: $\approx$20.6~s on a four-core laptop).
...
\begin{itemize}
  \item \textbf{KNN (K-Nearest Neighbours):} stores all 50{,}000 training
        ...
        This is the \emph{dominant cost}: about 75\% of total runtime
...
Boosting and bagging improve a \emph{single} base learner by re-weighting
or re-sampling training data. Here the three models have fundamentally
different inductive biases---KNN captures local geometry, NB assumes
feature independence, LR fits a global linear decision boundary---so
combining their diverse predictions provides a complementary error-reduction
effect that no single stronger model achieves without re-training. This
diversity also makes the ensemble more interpretable: each component
is a well-understood algorithm with clear failure modes.
...
%
\[
  20{,}000 \times 50{,}000 \times 20 \approx 2 \times 10^{10}
  \text{ multiply-accumulate operations}
\]
%
...
The sequential baseline processes one test sample at a time:
...
\begin{table}[H]
  ...
  \caption{\texttt{gprof} flat profile top entries (compiled \texttt{-O0 -pg},
           ...
           (\texttt{operator*}, \texttt{operator++}, etc.)\ are \texttt{-O0}
           ...
           NB and LR together account for $\approx$0.02\% --- effectively free.}
  \label{tab:gprof}
  \begin{tabular}{rll}
    \toprule
    \textbf{\% time} & \textbf{Function} & \textbf{Calls} \\
    \midrule
    75.07 & \texttt{knn\_distances\_tiled} (tiled distance kernel) & 15{,}000 \\
     1.57 & \texttt{std::\_\_heap\_select} (\texttt{partial\_sort} core) & 60{,}000 \\
     1.48 & \texttt{std::iota} (index reset per query)              & 60{,}000 \\
    $\approx$14 & iterator internals (\texttt{-O0} artefact, inlined at \texttt{-O2}) & --- \\
     0.02 & \texttt{nb\_predict\_proba}                            & 60{,}000 \\
...
\noindent
NB and LR together account for approximately 0.02\% of runtime --- effectively free.
...
\begin{enumerate}
  ...
  \item \textbf{Single global synchronisation point.} Aggregate is the only
        ...
        fraction. The profiler attributes $\approx 0.02\%$ to NB and LR combined;
...
and could in principle run concurrently. However, KNN accounts for
$\approx 75\%$ of per-sample cost (Section~\ref{sec:profiling}), so it
defines the intra-sample critical path by itself. Running NB and LR in
parallel with KNN would shorten per-sample time by at most $25\%$, while
forcing a thread fork/join on every one of the $N = 20{,}000$ samples
per run. The overhead dominates the gain. The correct level at which to
apply parallelism is \emph{across} sample groups, not within them.

\subsection{Agglomeration}
\label{sec:agglomeration}

The Partition phase identified $N = 20{,}000$ primitive tasks --- one per
test sample --- each fully independent. Assigning each primitive task to
its own MPI process is not feasible: the per-task communication overhead
(scatter + gather for a single sample: $20 \times 8 + 4 = 164$~bytes)
at a 1\,Gb/s link takes $\approx 1.3~\mu\text{s}$, while the compute
time per sample is $\approx 1.44$~ms, giving a granularity ratio of
$g \approx 1{,}100$ at rank granularity but would collapse toward
$g \approx 1$ at single-sample granularity --- communication cost and
compute cost would be comparable, making independent-task dispatch
inefficient.

The $N$ primitive tasks are therefore \textbf{agglomerated} into $p$
rank-level batches of $N/p$ samples each. At $p=4$ this gives
$g = T_\text{comp}/T_\text{comm} \approx 7{,}200\,\text{ms} /
6.4\,\text{ms} \approx 1{,}100$, confirming that each batch does
far more compute than it spends communicating. Within each batch,
the OpenMP-level agglomeration further groups per-sample iterations
into $t$ thread-sized blocks, with no additional communication cost
(threads share memory).

This agglomeration is also the only choice consistent with exact KNN:
each rank must hold the full training set to compute exact distances.
Finer agglomeration (more, smaller tasks) would multiply
scatter/gather messages without reducing the per-rank copy of
\texttt{X\_train}.
...
The contrast with training-set decomposition is instructive. If each rank
held only a shard of \texttt{X\_train}, it would need to compare its partial
distances with every other rank to find the global $k$ nearest neighbours.
Every rank would need a direct channel to every other rank, producing a
fully-connected graph with $O(p^2)$ edges. At $p=8$ that is already 28
connections instead of 7. Test-set decomposition avoids this entirely by
keeping each rank's inference self-contained.
...
\begin{table}[H]
  \centering
  \caption{The three MPI patterns and their roles.}
  \label{tab:comm}
  \footnotesize
  \begin{tabular}{lll}
    \toprule
    \textbf{Pattern} & \textbf{Type} & \textbf{Purpose} \\
    \midrule
    \texttt{MPI\_Scatter}
      & collective, blocking         & distribute test data \\
    \texttt{MPI\_Isend/Irecv + Waitall}
      & point-to-point, non-blocking & gather predictions at rank~0 \\
    \texttt{MPI\_Reduce}
      & collective, blocking         & sum correct counts for accuracy \\
    \bottomrule
  \end{tabular}
\end{table}
...
\subsection{Training Data Locality}

Every rank loads \texttt{X\_train} independently from disk rather than having
rank~0 broadcast it. Each rank's copy sits in its own memory region, so all $p$
ranks stream through training data in parallel without contending for the same
cache lines.
...
reduce phases while workers wait; (ii)~when $p$ does not divide $N_\text{test}$
evenly, the test set is trimmed to the nearest multiple of $p$ (at most $p-1$
dropped samples), introducing a negligible $< 0.01\%$ workload imbalance.
All other idle time is eliminated by the static equal-partition strategy.
...
$\approx 5\%$ of runtime (Table~\ref{tab:gprof}) and raise the effective
...
(above the $\approx$0.02\% NB+LR cost measured by the profiler; the gap is
